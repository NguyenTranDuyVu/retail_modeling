...
LOCAL_FILE_PATH = cwd / "data" / "ecommerce-data" / "item_dim.csv"
STAGE_FILE_NAME = f"item_dim.csv"
STAGE_NAME = "~"

def upload_to_snowflake_stage():
    snowflake_conn = BaseHook.get_connection("my_snowflake_conn")
...
with DAG(
    ...
    copy_task = SQLExecuteQueryOperator(
        task_id="copy_into_table", sql='item_load.sql', conn_id=SNOWFLAKE_CONN_ID,
        params={"STAGE_NAME": STAGE_NAME, 'STAGE_FILE_NAME': STAGE_FILE_NAME}
    )

    trigger_transform_item_task = TriggerDagRunOperator(
        task_id="trigger_transform_item",
        trigger_dag_id="transform_item_data",
        wait_for_completion=False,
    )

    create_table_task >> upload_task >> copy_task >> trigger_transform_item_task
